...
function ExecAttack(r1, r2, l1, l2, b1, b2, is_guard, blend_type, artsr1, artsr2, is_stealth_rolling)
    ...
    elseif request == ATTACK_REQUEST_BOTH_HEAVY then
		...
		if kind_R == 58 and style == HAND_RIGHT_BOTH and b2 == "W_AttackBothHeavyDash" and env(GetSpEffectID, 12000510) == FALSE and env(GetSpEffectID, 12000512) == FALSE then
			b2 = "W_AttackBothDash"
		end
		if kind_R == 65 then
...
function GetAttackMaxNumber(hand)
    ...
    elseif kind == WEAPON_CATEGORY_TORCH or kind == WEAPON_CATEGORY_LARGE_SWORD or kind == WEAPON_CATEGORY_FLAIL or kind == WEAPON_CATEGORY_LARGE_AX or kind == WEAPON_CATEGORY_LARGE_HAMMER or kind == WEAPON_CATEGORY_SPEAR or kind == WEAPON_CATEGORY_HALBERD or kind == WEAPON_CATEGORY_LARGE_CURVEDSWORD or kind == WEAPON_CATEGORY_SMALL_SHIELD or kind == WEAPON_CATEGORY_MIDDLE_SHIELD or kind == WEAPON_CATEGORY_LARGE_SCYTHE or kind == WEAPON_CATEGORY_WHIP or kind == 56 or kind == 57 then
        max_num = 4
    elseif kind == WEAPON_CATEGORY_STRAIGHT_SWORD or kind == WEAPON_CATEGORY_TWINBLADE or kind == WEAPON_CATEGORY_KATANA or kind == WEAPON_CATEGORY_AX or kind == WEAPON_CATEGORY_HAMMER or kind == WEAPON_CATEGORY_LARGE_RAPIER or kind == 58 then
        max_num = 5
    elseif kind == 66 or kind == 65 or kind == WEAPON_CATEGORY_SHORT_SWORD or kind == WEAPON_CATEGORY_CLAW or kind == WEAPON_CATEGORY_RAPIER or kind == WEAPON_CATEGORY_CURVEDSWORD or kind == WEAPON_CATEGORY_FIST or kind == 55 or kind == 53 or kind == 56 or kind == 62 or kind == 63 then
        max_num = 6
    end
...
function GetDualAttackMaxNumber(hand)
    ...
    if kind == WEAPON_CATEGORY_EXTRALARGE_AXHAMMER or kind == WEAPON_CATEGORY_LARGE_SPEAR or kind == WEAPON_CATEGORY_LARGE_AX or kind == WEAPON_CATEGORY_LARGE_HAMMER or kind == WEAPON_CATEGORY_SPEAR or kind == WEAPON_CATEGORY_HALBERD or kind == WEAPON_CATEGORY_LARGE_CURVEDSWORD or kind == WEAPON_CATEGORY_LARGE_SCYTHE or kind == WEAPON_CATEGORY_WHIP then
        max_num = 3
    elseif kind == WEAPON_CATEGORY_STRAIGHT_SWORD or kind == WEAPON_CATEGORY_TWINBLADE or kind == WEAPON_CATEGORY_AX or kind == WEAPON_CATEGORY_HAMMER or kind == WEAPON_CATEGORY_FLAIL or kind == WEAPON_CATEGORY_LARGE_RAPIER or kind == WEAPON_CATEGORY_SHORT_SWORD or kind == WEAPON_CATEGORY_RAPIER or kind == WEAPON_CATEGORY_CURVEDSWORD or kind == WEAPON_CATEGORY_CAT56 or kind == WEAPON_CATEGORY_CAT57 then
        max_num = 4
    elseif kind == WEAPON_CATEGORY_CLAW or kind == WEAPON_CATEGORY_FIST or kind == WEAPON_CATEGORY_CAT53 or kind == WEAPON_CATEGORY_CAT55 or kind == WEAPON_CATEGORY_CAT59 or kind == WEAPON_CATEGORY_CAT58 or WEAPON_CATEGORY_KATANA or kind == WEAPON_CATEGORY_EXTRALARGE_SWORD or kind == WEAPON_CATEGORY_LARGE_SWORD then
        max_num = 6
    end
    ...
    if is_dual == TRUE then
        ...
        if c_Style == HAND_RIGHT_BOTH or c_Style == HAND_LEFT_BOTH then
            ...
            elseif specialcategory == 182 or specialcategory == 207 then
                return TRUE
            elseif GetEquipType(hand, WEAPON_CATEGORY_TORCH, WEAPON_CATEGORY_STRAIGHT_SWORD, WEAPON_CATEGORY_TWINBLADE, WEAPON_CATEGORY_EXTRALARGE_SWORD, WEAPON_CATEGORY_RAPIER, WEAPON_CATEGORY_LARGE_RAPIER, WEAPON_CATEGORY_CURVEDSWORD) == TRUE or env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then
                return TRUE
            end
        end
...
function ExecMagic(quick_type, blend_type, is_ride)
    ...
    if env(IsMagicUseMenuOpening, f83_local1) == TRUE then
        ResetRequest()
        act(OpenMenuWhenUsingMagic, f83_local0)
        return TRUE
    end
    ...
    if env(IsMagicUseable, f83_local1, 0) == FALSE then
...
function ExecJump()
    if env(ActionRequest, ACTION_ARM_JUMP) == FALSE then
        return FALSE
    ...
    SetWeightIndex()
    if env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and c_Style == HAND_RIGHT_BOTH then
		return ExecEventAllBody("W_AttackDualLight4")
	end
    -- Jump: Overweight
...
function ExecHandChange(hand, is_force, blend_type, no_reset)
    ...
    act(Unknown2025,env(Unknown404))
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then  --变形斩剑戟
		if style == HAND_RIGHT then
			ExecEventAllBody("W_AttackRightSpecial1")
		elseif style == HAND_BOTH or style == HAND_RIGHT_BOTH then
			ExecEventAllBody("W_AttackRightSpecial2")
		end 
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 58 then  --变形斩洛阳
		if style == HAND_RIGHT and env(GetSpEffectID, 5033) == TRUE then
			ExecEventAllBody("W_AttackRightSpecial2")
		elseif style == HAND_RIGHT then
			ExecEventAllBody("W_AttackRightSpecial1")
		elseif (style == HAND_BOTH or style == HAND_RIGHT_BOTH) and env(GetSpEffectID, 5033) == TRUE then	
			ExecEventAllBody("W_AttackBothSpecial2")
		elseif style == HAND_BOTH or style == HAND_RIGHT_BOTH then	
			ExecEventAllBody("W_AttackBothSpecial1")
		
		end 
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and c_Style == HAND_RIGHT and c_Style ~= HAND_RIGHT_BOTH then
		ExecEventAllBody("W_AttackRightLightStealth")
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and c_Style == HAND_RIGHT_BOTH then
		ExecEventAllBody("W_AttackBothSpecial2")
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then
		act(AddSpEffect, 10015281)
		act(AddSpEffect, 10015300)
		ExecEventAllBody("W_AttackRightSpecial1")
	end 
    SetAIActionState()
    return TRUE
...
function ExecEvasion(backstep_limit, estep, is_usechainrecover)
    ...
    -- Stealth
	----------------------
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 10020061) == TRUE and env(ActionRequest, ACTION_ARM_L3) == TRUE then
		act(AddSpEffect, 10020062)
		return ExecEventAllBody("W_AttackBothLightStep")
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and c_Style == HAND_RIGHT_BOTH and env(ActionRequest, ACTION_ARM_L3) == TRUE then
		return FALSE
	end
    if env(ActionRequest, ACTION_ARM_L3) == TRUE and FALSE == c_IsStealth and env(GetSpEffectID, 10015280) == FALSE then
...
function ExecFallAttack()
    -- ERR: Powerstance fall R1
    if env(GetSpEffectID, 10015280) == TRUE then
		return FALSE
    elseif env(ActionRequest, ACTION_ARM_R1) == TRUE or env(ActionRequest, ACTION_ARM_R2) == TRUE or env(ActionRequest, ACTION_ARM_L1) == TRUE and (IsEnableDualWielding() == HAND_RIGHT and env(GetSpEffectID, 165000) == FALSE) then
        local style = c_Style
        local hand = HAND_RIGHT
        ...
        if env(ActionRequest, ACTION_ARM_R1) == TRUE or is_arrow == TRUE then
            ...
            -- ERR: Powerstance fall R1
            if IsEnableDualWielding() ~= -1 and env(GetSpEffectID, 165000) == TRUE then
        ...
        elseif env(ActionRequest, ACTION_ARM_R2) == TRUE then
            ...
            -- ERR: No more powerstance L1
        elseif env(ActionRequest, ACTION_ARM_L1) == TRUE and env(GetSpEffectID, 165000) == FALSE then
...
function Rolling_onUpdate()
	...
	-- Vigil
    act(SetMovementScaleMult, g_RollMovementMultiplier)
    --
...
function DefaultBackStep_onUpdate()
	...
	-- Vigil
    act(SetMovementScaleMult, g_BackstepMovementMultiplier)
    -- 
...
function Move_onUpdate()
    ...
    local base_speed = g_MoveMovementMultiplier
    local base_speed_percent = base_speed / 100
    
    -- Movement Speed Boost
    if env(GetSpEffectID, 151100) == TRUE then
        base_speed_percent = base_speed_percent * 1.2
    end
...
function AttackRightLight1_onUpdate()
	act(AddSpEffect, 103200)
	local r1 = "W_AttackRightLight2"
	local r2 = "W_AttackRightHeavy1SubStart"
	local b1 = "W_AttackBothLight2"
	local b2 = "W_AttackBothHeavy1Start"
    if env(GetEquipWeaponCategory, HAND_RIGHT) == 65 then  --dante
		if env(GetSpEffectID, 5033) == TRUE then
			r1 = "W_AttackRightLight4"
		end
		r2 = "W_AttackRightHeavy1End"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then  --cinderclaymore
		r2 = "W_AttackRightLight5"
		b1 = "W_AttackRightLight2"
		b2 = "W_AttackRightLight5"
	end
...
function AttackRightLight2_onUpdate()
    ...
    if IsEnableNextAttack(2, HAND_RIGHT) == TRUE then
		...
		if env(GetSpEffectID, 1626031) == TRUE then --法兰大剑单手轻击循环
			r1 = "W_AttackRightLight1"
		end
		if env(GetEquipWeaponCategory, HAND_RIGHT) == 65 then  --dante
			if env(GetSpEffectID, 5033) == TRUE then
				r1 = "W_AttackRightSpecial1"
			end
			r2 = "W_AttackRightHeavy1End"
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then  --cinderclaymore
			if env(GetSpEffectID, 5035) == TRUE then
				r1 = "W_AttackRightLight4"
			end
			r2 = "W_AttackRightHeavyCounter"
			b1 = "W_AttackRightLight3"
			b2 = "W_AttackRightHeavyCounter" 
		end
        if AttackCommonFunction(r1, r2, "W_AttackLeftLight1", "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 2) == TRUE then
            return
        end
...
function AttackRightLight3_onUpdate()
    ...
    if IsEnableNextAttack(3, HAND_RIGHT) == TRUE then
		...
		if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then  --法兰大剑 gp前顶
			b1 = "W_AttackRightHeavy1Start"
			l1 = "W_AttackBothHeavy2Start"
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == WEAPON_CATEGORY_KATANA then  --太刀单持连段循环
			r1 = "W_AttackRightLight2"
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then  --剑戟	
			r1 = "W_AttackRightLight2"
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 then  --dante
			r1 = "W_AttackRightLight1"
			r2 = "W_AttackRightHeavy1End"
        ...
        if AttackCommonFunction(r1, r2, l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 3) == TRUE then
            return
        end
    elseif AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", "W_AttackLeftLight1", "W_AttackLeftHeavy1", "W_AttackBothLight1", "W_AttackBothHeavy1Start", FALSE, TRUE, 0) == TRUE then
    end
...
function AttackRightLight4_onUpdate()
    ...
    if IsEnableNextAttack(4, HAND_RIGHT) == TRUE then
		...
		if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then --法兰大剑 寄鹰回旋
			b2 = "W_AttackBothLight4"
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == WEAPON_CATEGORY_KATANA then  --太刀 老一心侧身 后派生回斩
			b1 = "W_AttackRightLight5"
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 62 and c_Style == HAND_RIGHT_BOTH then  --剑戟冲锋二连枪HAND_RIGHT_BOTH 
			b1 = "W_AttackBothLight1"
			l1 = "W_AttackBothLight4"
			b2 = "W_AttackBothHeavy1Start"
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 58 then  --洛阳
			r1 = "W_AttackRightLight1"
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 then  --dante
		...
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then  --cinderclaymore
			...
			b1 = "W_AttackRightLight1"
			b2 = "W_AttackRightHeavyCounter"
			if c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 10015305) == TRUE then
				ZanshinCinder()
			end
		end
        if AttackCommonFunction(r1, r2, l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, atk_num) == TRUE then
            return
        end
...
function AttackRightLight5_onUpdate()
    ...
    if IsEnableNextAttack(5, HAND_RIGHT) == TRUE then
		...
		if env(GetEquipWeaponCategory, HAND_RIGHT) == WEAPON_CATEGORY_KATANA and c_Style == HAND_RIGHT_BOTH and 160 <= math.abs(GetVariable("MoveAngle")) then  --侧身回斩后拉摇杆派生 双持限定 仅居合
			b1 = "W_AttackBothLight4"
			b2 = "W_AttackBothLight5"
			atk_num = 4
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 62 and c_Style == HAND_RIGHT_BOTH then  --剑戟冲锋四连枪无蓄转二连
			b1 = "W_AttackBothLight1"
			l1 = "W_AttackBothLight4"
			b2 = "W_AttackBothHeavy1Start"
			if env(GetSpEffectID, 5033) == TRUE and env(ActionDuration, ACTION_ARM_L1) <= 0 then
				ExecEventAllBody("W_AttackRightLight4")
			end 
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then
			r1 = "W_AttackRightLight1"
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then  --cinderclaymore
			r1 = "W_AttackRightLight3"
			r2 = "W_AttackRightHeavy2End"
			b1 = "W_AttackRightLight3"
			b2 = "W_AttackRightHeavy2End"
			if c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 10015305) == TRUE then
				ZanshinCinder()
			end
		end
...
function AttackRightLight6_onUpdate()
	act(AddSpEffect, 103205)
    local r1 = "W_AttackRightLight1"
	local r2 = "W_AttackRightHeavy1Start"
	local l1 = "W_AttackLeftLight1"
	local b1 = "W_AttackBothLight1"
	local b2 = "W_AttackBothHeavy1Start"
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 62 and c_Style == HAND_RIGHT_BOTH then  --剑戟强化大炮
		l1 = "W_AttackBothLight4"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then  --cinderclaymore
		r1 = "W_AttackRightHeavy1Start"
		r2 = "W_AttackRightHeavy1End"
		b1 = "W_AttackRightHeavy1Start"
		b2 = "W_AttackRightHeavy1End"
		l1 = "W_AttackRightHeavy2Start"
		if env(GetSpEffectID, 5033) == TRUE then
			return ExecEventAllBody("W_AttackRightHeavy2Start")
		end
	end
    if AttackCommonFunction(r1, r2, l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 0) == TRUE then
    end
end

function AttackRightLightStep_onUpdate()
	act(AddSpEffect, 103206)
	...
	if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then --法兰大剑 剑匕三连
		b1 = "W_AttackRightHeavy1Start"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then  --cinderclaymore
		r1 = "W_AttackRightLight1"
		b1 = "W_AttackRightLight1"
	...
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and env(GetSpEffectID, 10015285) and c_Style == HAND_RIGHT_BOTH then
		b1 = "W_AttackDualLight1"
		b2 = "W_AttackBothLightStep"
		if env(GetSpEffectID, 5035) == TRUE then
			b1 = "W_AttackRightHeavySpecial1Start"
			b2 = "W_AttackRightHeavySpecial1End"
		end
	end
    if AttackCommonFunction(r1, r2, "W_AttackLeftLight1", "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 1) == TRUE then
    end
...
function AttackRightLightStealth_onUpdate()
    ...
    if AttackCommonFunction(r1, "W_AttackRightHeavy1Start", "W_AttackLeftLight1", "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 1) == TRUE then
    end
end

function AttackRightLightFastStep_onUpdate()
	act(AddSpEffect, 103208)
...
function AttackRightLightDash_onUpdate()
	...
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then
		r1 = "W_AttackRightLight1"
		b1 = "W_AttackRightLight1"
		r2 = "W_AttackRightLight5"
		b2 = "W_AttackRightLight5"
...
function AttackRightHeavyDash_onUpdate()
	...
	if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then  --法兰大剑 回旋火车
		b1 = "W_AttackBothLight2"
		b2 = "W_AttackBothHeavy1End"
		l1 = "W_AttackBothHeavy2Start"
		if math.abs(GetVariable("MoveAngle")) <= 20 then
			b2 = "W_AttackRightLightStep"
		end
		if env(GetSpEffectID, 5035) == TRUE then
			l1 = "W_AttackRightHeavy1Start"
		end
	end
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and env(GetSpEffectID, 10015285) and c_Style == HAND_RIGHT_BOTH then
		b1 = "W_AttackDualLight3"
		b2 = "W_AttackRightLightStep"
		if env(GetSpEffectID, 5035) == TRUE then
			b1 = "W_AttackRightHeavySpecial1Start"
			b2 = "W_AttackRightHeavySpecial1End"
    ...
    if AttackCommonFunction(r1, "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 1) == TRUE then
		return
    elseif 0 >= env(ActionDuration, ACTION_ARM_L1) and env(GetSpEffectID, 1626031) == TRUE and env(GetSpEffectID, 5033) == TRUE then --法兰大剑 蓄力窗口5033时未按住l1 转街舞1
        ExecEventAllBody("W_AttackBothHeavy1Start")
    end
end

function AttackRightWhileGuard_onUpdate()
	act(AddSpEffect, 103211)
...
function AttackRightHeavy1Start_onUpdate()
	act(AddSpEffect, 103212)
    act(SetSpecialInterpolation, 0, FALSE)
	if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT then --法兰大剑变形斩
		ExecEventAllBody("W_AttackRightHeavyCounter")
	end
	...
	if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then  --法兰大剑  大风车
		b1 = "W_AttackRightLightStep"
		b2 = "W_AttackBothHeavy2End"
		l1 = "W_AttackBothHeavy2End"
	end
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 65 then
		if env(GetSpEffectID, 5030) == TRUE and (0 >= env(ActionDuration, ACTION_ARM_R2) or env(ActionCancelRequest, ACTION_ARM_R2) == TRUE) then
			ExecEventAllBody("W_AttackRightHeavy1End")
			return
		elseif env(GetSpEffectID, 5033) == TRUE and 0 <= env(ActionDuration, ACTION_ARM_R2) then
			ExecEventAllBody("W_AttackRightHeavy2Start")
			return 
		end 
	end
	
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then  --cinderclaymore
		r1 = "W_AttackRightHeavy2End"
		b1 = "W_AttackRightHeavy2End"
		r2 = "W_AttackRightLight5"
		b2 = "W_AttackRightLight5"
		if c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 10015305) == TRUE then
			ZanshinCinder()
    ...
    if AttackCommonFunction(r1, r2, l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 1) == TRUE then
        return
    elseif 0 >= env(ActionDuration, ACTION_ARM_R2) and (env(GetGeneralTAEFlag, TAE_FLAG_CHARGING) == 1 or TRUE == env(GetSpEffectID, 100280)) then 
        if env(GetSpEffectID, 1626031) == FALSE and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 66 then  -- 禁蓄力
			ExecEventAllBody("W_AttackRightHeavy1End")
		end
    end
end
...
function AttackRightHeavy2Start_onUpdate()
	act(AddSpEffect, 103214)
	local r1 = "W_AttackRightLight1"
	local b1 = "W_AttackBothLight1"
	local r2 = "W_AttackRightHeavy1Start"
	local b2 = "W_AttackBothHeavy1Start"
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5033) == TRUE then -- dante
		r2 = "W_AttackRightHeavy2End"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then  --cinderclaymore
		r1 = "W_AttackRightHeavy2End"
		b1 = "W_AttackRightHeavy2End"
    ...
    if IsEnableSpecialAttack(HAND_RIGHT) == TRUE and env(ActionDuration, ACTION_ARM_R2) > 0 and env(ActionDuration, ACTION_ARM_SP_MOVE) > 0 and TRUE == env(GetSpEffectID, 100290) then
        ExecEventAllBody("W_AttackRightSpecial2")
        return
	elseif AttackCommonFunction(r1, r2, "W_AttackLeftLight1", "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 0) == TRUE then
        return
    elseif env(ActionDuration, ACTION_ARM_R2) <= 0 and (env(GetGeneralTAEFlag, TAE_FLAG_CHARGING) == 1 or TRUE == env(GetSpEffectID, 100280)) and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 66  then
		ExecEventAllBody("W_AttackRightHeavy2End")
    end
end

function AttackRightHeavy2End_onUpdate()
	act(AddSpEffect, 103215)
	local r1 = "W_AttackRightLight1"
	local b1 = "W_AttackRightHeavy1Start"
	local r2 = "W_AttackBothLight1"
	local b2 = "W_AttackBothHeavy1Start"
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then  --cinderclaymore
		r1 = "W_AttackRightLight1"
		b1 = "W_AttackRightLight1"
    ...
    if AttackCommonFunction(r1, r2, "W_AttackLeftLight1", "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 0) == TRUE then
    end
end
...
function AttackRightHeavySpecial1Start_onUpdate()
	...
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 62 and c_Style == HAND_RIGHT_BOTH then  --剑戟扫挑刺回归自由态
		l1 = "W_AttackBothLight4"
	end
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5030) == TRUE and 0 < env(ActionDuration, ACTION_ARM_R1) then  --dante
		ExecEventAllBody("W_AttackRightHeavySpecial1End")
		return
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5030) == TRUE and env(ActionDuration, ACTION_ARM_R1) <= 0 then  --dante
		ExecEventAllBody("W_AttackRightHeavySpecial2Start")
		return
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 then
		return
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 10015282) == TRUE then  -- cinderlance
		b1 = "W_AttackRightHeavySpecial2Start"
		b2 = "W_AttackBothHeavyDash"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 10015285) == TRUE then  --cinderclaymore
		b1 = "W_AttackDualLight1"
		b2 = "W_AttackBothLightStep"
	end 
	if AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy2Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 0, gen_trans) == TRUE then
        return
    elseif 0 >= env(ActionDuration, ACTION_ARM_R2) and (env(GetGeneralTAEFlag, TAE_FLAG_CHARGING) == 1 or env(GetSpEffectID, 100280) == TRUE) then
        if env(GetEquipWeaponCategory, HAND_RIGHT) ~= 62 and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 66 then  --剑戟特殊禁蓄力
			ExecEventAllBody("W_AttackRightHeavySpecial1End")
		end
    end
end
...
function AttackRightHeavySpecial1End_onUpdate()
	...
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 62 and c_Style == HAND_RIGHT_BOTH then  --剑戟前跳砸派生
		b1 = "W_AttackDualLight5"
		l1 = "W_AttackDualLight6"
		b2 = "W_AttackBothHeavy2End"
	end
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5030) == TRUE and env(ActionDuration, ACTION_ARM_R1) <= 0 or env(GetStamina) < 0 then  --dante
		ExecEventAllBody("W_AttackRightHeavySpecial2Start")
		return
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5030) == TRUE and 0 < env(ActionDuration, ACTION_ARM_R1) and env(GetStamina) > 0 then  --dante
		ExecEventAllBody("W_AttackRightHeavySpecial1End")
		return
	end
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 10015282) == TRUE then  -- cinderlance 
		b1 = "W_AttackRightHeavySpecial1Start"
		b2 = "W_AttackBothHeavyDash"
		if c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 10015305) == TRUE then
			ZanshinCinder()
		end
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and env(GetSpEffectID, 10015285) and c_Style == HAND_RIGHT_BOTH then
		b1 = "W_AttackDualLight1"
		b2 = "W_AttackBothLightStep"
	end
    if AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy2Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 0, gen_trans) == TRUE then
...
function AttackRightHeavySpecial2Start_onUpdate()
	...
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and env(GetSpEffectID, 10015282) == TRUE and c_Style == HAND_RIGHT_BOTH then  -- cinderlance 
		b1 = "W_AttackRightHeavySpecial1End"
		b2 = "W_AttackBothHeavyDash"
	end
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and env(GetSpEffectID, 10015285) == TRUE and c_Style == HAND_RIGHT_BOTH then  --cinderclaymore
		b1 = "W_AttackDualLight2"
		b2 = "W_AttackRightLightStealth"
		if env(GetSpEffectID, 5035) == TRUE then
			b1 = "W_AttackRightHeavySpecial1Start"
			b2 = "W_AttackRightHeavySpecial1End"
		end
	end
    if AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 0) == TRUE then
        return
    elseif 0 >= env(ActionDuration, ACTION_ARM_R2) and (env(GetGeneralTAEFlag, TAE_FLAG_CHARGING) == 1 or TRUE == env(GetSpEffectID, 100280)) then
        if env(GetEquipWeaponCategory, HAND_RIGHT) ~= 62 and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 65 and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 66 then  --禁蓄力
...
function AttackRightHeavySpecial2End_onUpdate()
	act(AddSpEffect, 103219)
	local r1 = "W_AttackRightLight1"
	local r2 = "W_AttackRightHeavy1Start"
	local b1 = "W_AttackBothLight1"
	local b2 = "W_AttackBothHeavy1Start"
	local l1 = "W_AttackLeftLight1"
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 62 and c_Style == HAND_RIGHT_BOTH then  --剑戟后跳砸派生
		b1 = "W_AttackRightHeavySpecial1Start"
		l1 = "W_AttackRightLight5"
		b2 = "W_AttackRightLight6"
	end
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and c_Style == HAND_RIGHT then  -- dante
		r1 = "W_AttackRightHeavySpecial1Start"
		r2 = "W_AttackDualLight1"
	end
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 10015282) == TRUE then  -- cinderlance 
		b1 = "W_AttackRightHeavySpecial1Start"
		b2 = "W_AttackBothHeavyDash"
	end
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 10015285) == TRUE then  --cinderclaymore
		b1 = "W_AttackDualLight2"
		b2 = "W_AttackRightHeavyDash"
		if env(GetSpEffectID, 5035) == TRUE then
			b1 = "W_AttackRightHeavySpecial1Start"
			b2 = "W_AttackRightHeavySpecial1End"
		end
...
function AttackRightSpecial1_onUpdate()
	act(AddSpEffect, 103220)
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5030) == TRUE and 0 < env(ActionDuration, ACTION_ARM_R1) then
		ExecEventAllBody("W_AttackRightSpecial2")
		return
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 5030) == TRUE then --cinderclaymore
		if env(ActionDuration, ACTION_ARM_L2) <= 0 or env(ActionCancelRequest, ACTION_ARM_L2) == TRUE then  --sword
			act(AddSpEffect, 10015281)
		elseif 0 < env(ActionDuration, ACTION_ARM_R1) then  --katana
			act(AddSpEffect, 10015283)
		elseif 0 < env(ActionDuration, ACTION_ARM_R2) then  --lance
			act(AddSpEffect, 10015282)
		elseif 0 < env(ActionDuration, ACTION_ARM_L1) then  --magic
			act(AddSpEffect, 10015284)
		end
    end
...
function AttackRightSpecial2_onUpdate()
	act(AddSpEffect, 103221)
	local r1 = "W_AttackRightBackstep"
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5035) == TRUE then
		ExecEventAllBody("W_AttackRightLight5")
		return
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and c_Style == HAND_RIGHT_BOTH then -- cinderclaymore
		if env(GetSpEffectID, 5033) == TRUE and (env(ActionDuration, ACTION_ARM_L2) <= 0 or env(ActionCancelRequest, ACTION_ARM_L2) == TRUE or env(ActionDuration, ACTION_ARM_L1) > 0 or env(ActionDuration, ACTION_ARM_R1) > 0 or env(ActionDuration, ACTION_ARM_R2) > 0) then
			return ExecEventAllBody("W_AttackRightSpecial1")
		end
	end
    if AttackCommonFunction(r1, "W_AttackRightHeavy1Start", "W_AttackLeftLight1", "W_AttackLeftHeavy1", "W_AttackBothLight1", "W_AttackBothHeavy1Start", FALSE, TRUE, 0) == TRUE then
    end
	return
end

function AttackRightBackstep_onUpdate()
	act(AddSpEffect, 103222)
    local r1 = "W_AttackRightLightSubStart"
    local r2 = "W_AttackRightHeavy1Start"
	local b1 = "W_AttackBothLight1"
	local b2 = "W_AttackBothHeavy1Start"
...
function AttackDualLight1_onUpdate()
	...
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 62 and c_Style == HAND_RIGHT_BOTH then  --剑戟	
		b1 = "W_AttackBothLight3"
		b2 = "W_AttackBothHeavy1End"
		l1 = "W_AttackDualLight2"
		if 160 < math.abs(GetVariable("MoveAngle")) then
			b1 = "W_AttackRightHeavySpecial2End"
		elseif env(ActionDuration, ACTION_ARM_L1) > 0 then 
			b1 = "W_AttackBothHeavySpecial1Start"
		elseif env(GetSpEffectID, 5035) > 0 then 
			b2 = "W_AttackRightHeavySpecial1End"
		end
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 58 and c_Style == HAND_RIGHT_BOTH then  --洛阳
		l1 = "W_AttackBothLight1"
		b1 = "W_AttackDualLight2"
		b2 = "W_AttackBothHeavySpecial1End"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and c_Style == HAND_RIGHT then  -- dante
		r2 = "W_AttackDualLight2"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and env(GetSpEffectID, 10015285) == TRUE and c_Style == HAND_RIGHT_BOTH then  --cinderclaymore
		b1 = "W_AttackDualLight2"
		b2 = "W_AttackRightLightStealth"
		if env(GetSpEffectID, 5035) == TRUE then
			b1 = "W_AttackRightHeavySpecial1Start"
...
function AttackDualLight2_onUpdate()
	...
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 62 and c_Style == HAND_RIGHT_BOTH then  --剑戟	
		b1 = "W_AttackBothHeavySpecial2End"
		b2 = "W_AttackBothHeavy1End"
		l1 = "W_AttackBothLight5"	
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 58 and c_Style == HAND_RIGHT_BOTH then  --洛阳
		l1 = "W_AttackBothLight3"
		b1 = "W_AttackDualLight3"
		b2 = "W_AttackBothHeavySpecial1Start"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and c_Style == HAND_RIGHT then  -- dante
		r2 = "W_AttackDualLight3"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and env(GetSpEffectID, 10015285) == TRUE and c_Style == HAND_RIGHT_BOTH then  --cinderclaymore
		b1 = "W_AttackDualLight3"
		b2 = "W_AttackDualLight4"
		if env(GetSpEffectID, 5035) == TRUE then
			b1 = "W_AttackRightHeavySpecial1Start"
			b2 = "W_AttackRightHeavySpecial1End"
		end
...
function AttackDualLight3_onUpdate()
	...
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 58 and c_Style == HAND_RIGHT_BOTH then  --洛阳
		l1 = "W_AttackBothLight4"
		b1 = "W_AttackDualLight4"
		if env(GetSpEffectID, 12000510) == TRUE or env(GetSpEffectID, 12000512) == TURE then
			b2 = "W_AttackBothHeavySpecial2Start"
		end
	elseif GetEquipType(HAND_RIGHT, WEAPON_CATEGORY_KATANA) == TRUE and GetEquipType(HAND_LEFT, WEAPON_CATEGORY_KATANA) == TRUE and c_Style == HAND_RIGHT and (env(GetSpEffectID, 103268) == TRUE or env(GetSpEffectID, 103269) == TRUE) == TRUE then	
		if 160 <= math.abs(GetVariable("MoveAngle")) then --双太刀多才多艺8 后拉摇杆派生雷神
			b1 = "W_AttackDualStealth"	
			l1 = "W_AttackDualStealth"	
		end 	
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 10015285) == TRUE then  --cinderclaymore
		b1 = "W_AttackDualLight2"
    ...
    if AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 1) == TRUE then
    end
end

function AttackDualLight4_onUpdate()
	act(AddSpEffect, 103226)
	...
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 62 and c_Style == HAND_RIGHT_BOTH then  --剑戟	
		b1 = "W_AttackRightHeavySpecial1Start"
		b2 = "W_AttackBothHeavy1End"
		l1 = "W_AttackRightHeavySpecial2Start"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 58 and c_Style == HAND_RIGHT_BOTH then  --洛阳
		b1 = "W_AttackDualLight1"
		b2 = "W_AttackBothHeavy1Start"
		l1 = "W_AttackBothLight1"
	...
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5033) == TRUE then
		return ExecEventAllBody("W_AttackDualLight5")
	end
    if AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 1) == TRUE then
    end
end
...
function AttackDualLight5_onUpdate()
	...
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5033) == TRUE then
		if env(ActionDuration, ACTION_ARM_JUMP) > 0 then
			return ExecEventAllBody("W_AttackDualLight5")
		elseif env(ActionDuration, ACTION_ARM_JUMP) <= 0 or env(ActionCancelRequest, ACTION_ARM_JUMP) == TRUE then
			return ExecEventAllBody("W_AttackDualLight6")
		end
...
function AttackDualLight6_onUpdate()
	...
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 62 and c_Style == HAND_RIGHT_BOTH then  --剑戟	
		...
		if env(ActionDuration, ACTION_ARM_R1) > 0 then
			b1 = "W_AttackRightHeavySpecial1Start"
			b2 = "W_AttackRightHeavySpecial2Start"
		end
	end
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and c_Style == HAND_RIGHT_BOTH then  --cinderclaymore
		b1 = "W_AttackRightLight1"
		b2 = "W_AttackRightLight5"
	end
    if AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 1) == TRUE then
...
function AttackDualDash_onUpdate()
	...
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 10015285) then  --cinderclaymore
		b1 = "W_AttackDualLight1"
		b2 = "W_AttackBothLightStep"
	end
    if AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 1) == TRUE then
...
function AttackDualStealth_onUpdate()
	act(AddSpEffect, 103231)
	local b1 = "W_AttackBothLight1"
	local b2 = "W_AttackBothHeavy1Start"
    local l1 = "W_AttackLeftLight2"
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 62 and c_Style == HAND_RIGHT_BOTH then  --剑戟	
		if env(GetSpEffectID, 12000501) == TRUE then
			act(AddSpEffect, 12000502) 
		end
		l1 = "W_AttackBothLight4"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 58 and c_Style == HAND_RIGHT_BOTH then  --洛阳
		l1 = "W_AttackBothLight1"
...
function AttackDualBackStep_onUpdate()
	...
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 10015285) then  --cinderclaymore
		...
		b2 = "W_AttackBothLightStep"
		if env(GetSpEffectID, 5033) == TRUE then
			if env(ActionDuration, ACTION_ARM_L1) <= 0 or env(ActionCancelRequest, ACTION_ARM_L1) == TRUE then
				return ExecEventAllBody("W_AttackDualDash")
			elseif env(ActionDuration, ACTION_ARM_R1) > 0 then
				return ExecEventAllBody("W_AttackDualRolling")
			elseif env(ActionDuration, ACTION_ARM_R2) > 0 then
				return ExecEventAllBody("W_AttackDualStealth")
			end
		end
	end
...
function AttackBothLight1_onUpdate()
	...
	if env(GetSpEffectID, 1626031) == TRUE then --法兰大剑 双持轻击1
		b2 = "W_AttackBothHeavy1End"
		l1 = "W_AttackBothHeavy2Start"
		if 160 <= math.abs(GetVariable("MoveAngle")) then
			b2 = "W_AttackRightBackstep"
		end
		if env(GetSpEffectID, 5035) == TRUE then  --延迟派生大风车
			l1 = "W_AttackRightHeavy1Start"
		end
	elseif GetEquipType(HAND_RIGHT, WEAPON_CATEGORY_KATANA) == TRUE and c_Style == HAND_RIGHT_BOTH and 0 < env(ActionDuration, ACTION_ARM_L1) then
		if 0 < env(ActionDuration, ACTION_ARM_L1) then
			b1 = "W_AttackRightLight4"
		elseif 160 <= math.abs(GetVariable("MoveAngle")) then
			b1 = "W_AttackBothLight4"
			b2 = "W_AttackBothLight5"
		end
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then --剑戟
		b1 = "W_AttackBothLight2"
		l1 = "W_AttackBothLight6"
		b2 = "W_AttackBothHeavy1End"
		if env(GetSpEffectID, 5035) == TRUE then
			b1 = "W_AttackDualLight1"
			b2 = "W_AttackRightLight6"
		end
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 63 then --古达
		b1 = "W_AttackBothLight2"
		l1 = "W_AttackBothLight3"
		b2 = "W_AttackBothLight6"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 58 and c_Style == HAND_RIGHT_BOTH then  --洛阳
		l1 = "W_AttackBothLight2"
		b1 = "W_AttackDualLight1"
		b2 = "W_AttackBothHeavySpecial1Start"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and c_Style == HAND_RIGHT_BOTH then  --cinderclaymore
		b2 = "W_AttackBothHeavy1Start"
		if math.abs(GetVariable("MoveAngle")) > 160 then
			b1 = "W_AttackBothSpecial1"
		end
	end
    if AttackCommonFunction("W_AttackRightLight2", "W_AttackRightHeavy1Start", l1, l2, b1, b2, FALSE, TRUE, 1) == TRUE then
    end
end

function AttackBothLight2_onUpdate()
	act(AddSpEffect, 103234)
    if IsEnableNextAttack(2, HAND_RIGHT) == TRUE then
        local b1 = "W_AttackBothLight3"
		local b2 = "W_AttackBothHeavy1Start"
		...
		if env(GetSpEffectID, 1626031) == TRUE then --法兰大剑 双持轻击2
		elseif GetEquipType(HAND_RIGHT, WEAPON_CATEGORY_KATANA) == TRUE and c_Style == HAND_RIGHT_BOTH then
			if 0 < env(ActionDuration, ACTION_ARM_L1) then
				b1 = "W_AttackRightLight4"
			elseif 160 <= math.abs(GetVariable("MoveAngle")) then
				b1 = "W_AttackBothLight4"
        ...
        if AttackCommonFunction("W_AttackRightLight3", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 2) == TRUE then
            return
        end
    elseif AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", "W_AttackLeftLight1", "W_AttackLeftHeavy1", "W_AttackBothLight1", "W_AttackBothHeavy1Start", FALSE, TRUE, 0) == TRUE then
    end
...
function AttackBothLight3_onUpdate()
    ...
    if IsEnableNextAttack(3, HAND_RIGHT) == TRUE then
		...
		if env(GetSpEffectID, 1626031) == TRUE then --法兰大剑 双持轻击3循环
			b1 = "W_AttackBothLight1"
			b2 = "W_AttackBothHeavyDash"
		elseif GetEquipType(HAND_RIGHT, WEAPON_CATEGORY_KATANA) == TRUE and c_Style == HAND_RIGHT_BOTH and 0 < env(ActionDuration, ACTION_ARM_L1) then
			if 0 < env(ActionDuration, ACTION_ARM_L1) then
				b1 = "W_AttackRightLight4"
			elseif 160 <= math.abs(GetVariable("MoveAngle")) then  --太刀后拉摇杆三种派生 双持限定
				b1 = "W_AttackBothLight4"
				b2 = "W_AttackBothLight5"
			else  --太刀双持平砍连段循环
				b1 = "W_AttackBothLight2"
        ...
        if AttackCommonFunction("W_AttackRightLight4", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 3) == TRUE then
            return
        end
    elseif AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", "W_AttackLeftLight1", "W_AttackLeftHeavy1", "W_AttackBothLight1", "W_AttackBothHeavy1Start", FALSE, TRUE, 0) == TRUE then
    end
...
function AttackBothLight4_onUpdate()
	...
	if IsEnableNextAttack(4, HAND_RIGHT) == TRUE then
		...
		if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then --车中途打断
			b1 = "W_AttackRightLightStep"
			b2 = "W_AttackBothBackstep"
			l1 = "W_AttackBothDash"
			atk_num = 3
		elseif GetEquipType(HAND_RIGHT, WEAPON_CATEGORY_KATANA) == TRUE and c_Style == HAND_RIGHT_BOTH and 0 < env(ActionDuration, ACTION_ARM_L1) then
			b1 = "W_AttackRightLight4"
		elseif GetEquipType(HAND_RIGHT, WEAPON_CATEGORY_KATANA) == TRUE and c_Style == HAND_RIGHT_BOTH then  --太刀居合一字恢复常规连段1
			b1 = "W_AttackBothLight1"
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then --剑戟
			b1 = "W_AttackBothLight3"
			b2 = "W_AttackBothHeavy1End"
		...
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 63 then --古达
			...
			if env(ActionDuration, ACTION_ARM_L1) > 0 then
				b2 = "W_AttackBothHeavySpecial2End"
			end	
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 58 and c_Style == HAND_RIGHT_BOTH then  --洛阳
			l1 = "W_AttackBothLight1"
			b1 = "W_AttackDualLight1"
		...
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 then
			b1 = "W_AttackBothLight1"
			b2 = "W_AttackDualRolling"
			l1 = "W_AttackBothLight5"
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and c_Style == HAND_RIGHT_BOTH and env(GetSpEffectID, 10015284) == TRUE then  --cinderclaymore
			b1 = "W_AttackBothLight4"
			b2 = "W_AttackBothLightStealth"
		end 	
        if AttackCommonFunction("W_AttackRightLight5", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, atk_num) == TRUE then
            return
        end
    elseif AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", "W_AttackLeftLight1", "W_AttackLeftHeavy1", "W_AttackBothLight1", "W_AttackBothHeavy1Start", FALSE, TRUE, 0) == TRUE then
    end
...
function AttackBothLight5_onUpdate()
    ...
    if IsEnableNextAttack(5, HAND_RIGHT) == TRUE then
		...
		if GetEquipType(HAND_RIGHT, WEAPON_CATEGORY_KATANA) == TRUE and c_Style == HAND_RIGHT_BOTH and 0 < env(ActionDuration, ACTION_ARM_L1) then
			b1 = "W_AttackRightLight4"
			atk_num = 4
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == WEAPON_CATEGORY_KATANA and c_Style == HAND_RIGHT_BOTH then  --太刀居合十字恢复常规连段1
			b1 = "W_AttackBothLight1"
		elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then --剑戟
			b1 = "W_AttackDualLight5"
			b2 = "W_AttackBothHeavy2End"
        ...
        if AttackCommonFunction("W_AttackRightLight6", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, atk_num) == TRUE then
            return
        end
    elseif AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", "W_AttackLeftLight1", "W_AttackLeftHeavy1", "W_AttackBothLight1", "W_AttackBothHeavy1Start", FALSE, TRUE, 0) == TRUE then
    end
...
function AttackBothLight6_onUpdate()
	act(AddSpEffect, 103238)
    local b1 = "W_AttackBothLight1"
	local b2 = "W_AttackBothHeavy1SubStart"
	local l1 = "W_AttackBothLeft2"
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then --剑戟
		b1 = "W_AttackDualLight4"
		b2 = "W_AttackBothHeavy1End"
		l1 = "W_AttackRightHeavySpecial1End"
		if env(GetSpEffectID, 5035) == TRUE then
			b1 = "W_AttackBothHeavySpecial1Start" 
		end
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 63 then --古达
...
function AttackBothHeavy1Start_onUpdate()
	...
	local b2 = "W_AttackBothHeavy2Start"
	local l1 = "W_AttackLeftLight1"
	if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then  --法兰大剑 街舞1
		b1 = "W_AttackBothLight2"
		b2 = "W_AttackBothHeavy1End"
		l1 = "W_AttackBothHeavy2Start"
		if math.abs(GetVariable("MoveAngle")) <= 20 then
	...
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5033) == TRUE then
		return ExecEventAllBody("W_AttackBothHeavy1End")
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then --cinderclaymore
		b1 = "W_AttackBothLight2"
		if math.abs(GetVariable("MoveAngle")) > 160 then
			b1 = "W_AttackBothSpecial1"
    ...
    if AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy2Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 1) == TRUE then
        return
	elseif c_style == 58 and env(GetSpEffectID, 12000510) == FALSE and env(GetSpEffectID, 12000512) == FALSE then
		ExecEventAllBody("W_AttackBothHeavy1End")
    elseif 0 >= env(ActionDuration, ACTION_ARM_R2) and (env(GetGeneralTAEFlag, TAE_FLAG_CHARGING) == 1 or TRUE == env(GetSpEffectID, 100280)) then
        if (env(GetEquipWeaponCategory, HAND_RIGHT) ~= 65 and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 62 and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 66 and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 63 and env(GetSpEffectID, 1626031) == FALSE) and c_Style == HAND_RIGHT_BOTH then  --特殊武器禁用重蓄
			ExecEventAllBody("W_AttackBothHeavy1End")
		end
	end
end
...
function AttackBothHeavy1End_onUpdate()
	...
	if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then  --法兰大剑  二连斩
		b1 = "W_AttackRightLightStep"
		b2 = "W_AttackBothHeavy2End"
		l1 = "W_AttackBothHeavy1Start"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then	  --剑戟	
		b1 = "W_AttackDualLight5"
		l1 = "W_AttackDualLight6"
		b2 = "W_AttackBothHeavy2End"
	...
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5033) == TRUE then
		if env(ActionDuration, ACTION_ARM_R2) > 0 then
			return ExecEventAllBody("W_AttackBothHeavy1End")
		elseif env(ActionDuration, ACTION_ARM_R2) <= 0 or env(ActionCancelRequest, ACTION_ARM_R2) == TRUE then
			return ExecEventAllBody("W_AttackBothHeavy2Start")
		end
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then --cinderclaymore
		b1 = "W_AttackBothLight1"
		b2 = "W_AttackBothHeavy2End"
...
function AttackBothHeavy2Start_onUpdate()
	act(AddSpEffect, 103241)
	local b1 = "W_AttackBothLight1"
	local b2 = "W_AttackBothHeavy1Start"
	local l1 = "W_AttackBothLeft1"
	if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then--法兰大剑  街舞2
		b1 = "W_AttackRightLightStep"
		b2 = "W_AttackBothHeavy2End"
		l1 = "W_AttackBothHeavy2End"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then  --剑戟	
    ...
    if IsEnableSpecialAttack(HAND_RIGHT) == TRUE and env(ActionDuration, ACTION_ARM_R2) > 0 and env(ActionDuration, ACTION_ARM_SP_MOVE) > 0 and TRUE == env(GetSpEffectID, 100290) then
        ExecEventAllBody("W_AttackBothSpecial2")
        return
    elseif AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 0) == TRUE then
        return
	elseif c_Style == 58 and env(GetSpEffectID, 12000510) == FALSE and env(GetSpEffectID, 12000512) == FALSE then
    ...
    elseif env(ActionDuration, ACTION_ARM_R2) <= 0 and (env(GetGeneralTAEFlag, TAE_FLAG_CHARGING) == 1 or TRUE == env(GetSpEffectID, 100280)) then
        if env(GetEquipWeaponCategory, HAND_RIGHT) ~= 65 and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 62 and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 66 and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 63 and env(GetSpEffectID, 1626031) == FALSE then  --特殊武器禁用重蓄
			ExecEventAllBody("W_AttackBothHeavy2End")
		end
    end
end
...
function AttackBothHeavy2End_onUpdate()
	act(AddSpEffect, 103242)
    local b1 = "W_AttackBothLight1"
	local b2 = "W_AttackBothHeavy1Start"
	local l1 = "W_AttackBothLeft1"
	if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then--法兰大剑  街舞3
		b1 = "W_AttackBothLight1"
		b2 = "W_AttackBothHeavy1End"
		l1 = "W_AttackBothHeavy1Start"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then  --剑戟	
...
function AttackBothHeavySpecial1Start_onUpdate()
	...
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 63 then --古达
		...
		b2 = "W_AttackBothHeavySpecial2End"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5033) == TRUE  then
		return ExecEventAllBody("W_AttackBothHeavySpecial1End")
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and env(GetSpEffectID, 10015282) == TRUE then --cinderclaymore
		if (env(ActionDuration, ACTION_ARM_L1) < 0 or env(ActionCancelRequest, ACTION_ARM_L1) == TRUE) and env(GetSpEffectID, 5033) == TRUE then
			return ExecEventAllBody("W_AttackBothHeavySpecial1End")
		end
		b1 = "W_AttackRightHeavySpecial1Start"
		b2 = "W_AttackBothHeavyDash"
    ...
    if AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 0) == TRUE then
        return
    elseif 0 >= env(ActionDuration, ACTION_ARM_R2) and (env(GetGeneralTAEFlag, TAE_FLAG_CHARGING) == 1 or TRUE == env(GetSpEffectID, 100280)) then
        if env(GetEquipWeaponCategory, HAND_RIGHT) ~= 65 and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 62 and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 66 and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 63 and env(GetEquipWeaponCategory, HAND_RIGHT) ~= 58 then  --特殊武器禁用重蓄
...
function AttackBothHeavySpecial1End_onUpdate()
	...
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5033) == TRUE then
		...
		elseif env(ActionDuration, ACTION_ARM_L2) <= 0 or env(ActionCancelRequest, ACTION_ARM_L2) == TRUE then
			return ExecEventAllBody("W_AttackBothHeavySpecial2Start")
		end
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and env(GetSpEffectID, 10015282) == TRUE then --cinderclaymore
		b1 = "W_AttackRightHeavySpecial1Start"
		b2 = "W_AttackBothHeavyDash"
...
function AttackBothHeavySpecial2End_onUpdate()
	...
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and env(GetSpEffectID, 10015283) == TRUE and env(GetSpEffectID, 5033) == TRUE then  --cinderclaymore
		if env(ActionDuration, ACTION_ARM_L1) <= 0 or env(ActionCancelRequest, ACTION_ARM_L1) == TRUE then
			return ExecEventAllBody("W_AttackBothHeavySpecial2Start")
		elseif env(ActionDuration, ACTION_ARM_R1) > 0 then
			return ExecEventAllBody("W_AttackBothHeavySpecial1Start")
...
function AttackBothSpecial1_onUpdate()
    ...
    if AttackCommonFunction("W_AttackRightBackstep", "W_AttackRightHeavy1Start", "W_AttackBothLeft1", "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 0) == TRUE then
    end
end

function AttackBothSpecial2_onUpdate()
	act(AddSpEffect, 103248)
	if env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and env(GetSpEffectID, 10015284) == TRUE and env(GetSpEffectID, 5033) == TRUE then  --cinderclaymore
		if env(ActionDuration, ACTION_ARM_R1) > 0 then
			return ExecEventAllBody("W_AttackBothLight4")
		elseif env(ActionDuration, ACTION_ARM_R2) > 0 then
			return ExecEventAllBody("W_AttackBothHeavyCounter")
		end
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 and env(GetSpEffectID, 5033) == TRUE then
		return ExecEventAllBody("W_AttackBothSpecial1")
	end
...
function AttackBothDash_onUpdate()
	...
	if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then --法兰大剑  跑攻
		b1 = "W_AttackBothLight2"
		l1 = "W_AttackBothHeavy2Start"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then --剑戟
		b1 = "W_AttackDualLight5"
	...
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then --cinderclaymore
		b1 = "W_AttackRightHeavySpecial2Start"
		b2 = "W_AttackRightHeavySpecial2End"
	end
    if AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 1) == TRUE then
    end
end
...
function AttackBothHeavyDash_onUpdate()
	...
	if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then  --法兰大剑 狼跳超循环
		b2 = "W_AttackBothHeavyDash"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then --剑戟
		b2 = "W_AttackRightHeavyCounter"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 63 then --古达
	...
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then --cinderclaymore
		b1 = "W_AttackRightHeavySpecial2Start"
		b2 = "W_AttackRightHeavySpecial2End"
		if env(GetSpEffectID, 5033) == TRUE and env(ActionDuration, ACTION_ARM_R2) <= 0 then
...
function AttackBothLightStep_onUpdate()
	...
	if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then --法兰大剑  滚攻
		b1 = "W_AttackBothLight2"
		l1 = "W_AttackBothHeavy2Start"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 62 then --剑戟
		b1 = "W_AttackDualLight5"
	...
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 and env(GetSpEffectID, 10015285) == TRUE and c_Style == HAND_RIGHT_BOTH then
		b1 = "W_AttackDualLight2"
		b2 = "W_AttackRightHeavyDash"
		if env(GetSpEffectID, 5033) == TRUE and env(ActionDuration, ACTION_ARM_R2) < 0 then 
			return ExecEventAllBody("W_AttackDualLight5")
...
function AttackBothLightStealth_onUpdate()
	...
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 65 then
		if env(IsLanding) == TRUE then
			return ExecEventAllBody("W_AttackBothBackstep")
		elseif env(GetSpEffectID, 5033) == TRUE then
			return ExecEventAllBody("W_AttackBothLightStealth")
		end
		
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then --cindermagic
		b1 = "W_AttackBothLight5"
		b2 = "W_AttackBothLightStealth"
	end
    if AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 1) == TRUE then
...
function AttackBothBackstep_onUpdate()
	...
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 63 then --古达
		...
		l1 = "W_AttackBothHeavyCounter"
		b2 = "W_AttackBothHeavy2Start"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 66 then  --cinderlance
		b1 = "W_AttackRightHeavySpecial2Start"
		b2 = "W_AttackRightHeavySpecial2End"
	end
    if AttackCommonFunction("W_AttackRightLight1", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 1) == TRUE then
...
function AttackBothHeavyCounter_onUpdate()
	...
	if env(GetSpEffectID, 1626031) == TRUE and c_Style == HAND_RIGHT_BOTH then --地火车中途打断
		b1 = "W_AttackRightLightStep"
		b2 = "W_AttackBothBackstep"
		l1 = "W_AttackBothDash"
	elseif env(GetEquipWeaponCategory, HAND_RIGHT) == 63 then --古达
    ...
    if AttackCommonFunction("W_AttackRightLight2", "W_AttackRightHeavy1Start", l1, "W_AttackLeftHeavy1", b1, b2, FALSE, TRUE, 1) == TRUE then
    end
end
...
function SwordArtsOneShot_onUpdate()
    ...
    elseif env(ActionDuration, ACTION_ARM_L2) <= 0 then
        if TRUE == env(GetSpEffectID, 100285) then
            local idle_cat = env(GetStayAnimCategory)
            local wep_cat = env(GetEquipWeaponCategory, c_SwordArtsHand)
            local f433_local0 = GetSwordArtsDiffCategory(c_SwordArtsID, idle_cat, wep_cat)
            local arts_idx = 0
            if f433_local0 == 47 then
                arts_idx = 1
            elseif f433_local0 == 48 then
                arts_idx = 2
            end
...
function SwordArtsOneShotShieldLeft_onUpdate()
    ...
    if env(ActionDuration, ACTION_ARM_L2) <= 0 then
        local idle_cat = env(GetStayAnimCategory)
        local wep_cat = env(GetEquipWeaponCategory, c_SwordArtsHand)
        local f439_local0 = GetSwordArtsDiffCategory(c_SwordArtsID, idle_cat, wep_cat)
        local arts_idx = 0
        if f439_local0 == 47 then
            arts_idx = 1
        elseif f439_local0 == 48 then
            arts_idx = 2
        end
...
function SwordArtsHalfOneShotShieldLeft_Upper_onUpdate()
    ...
    local idle_cat = env(GetStayAnimCategory)
    local wep_cat = env(GetEquipWeaponCategory, c_SwordArtsHand)
    local f440_local0 = GetSwordArtsDiffCategory(c_SwordArtsID, idle_cat, wep_cat)
    local arts_idx = 0
    if f440_local0 == 47 then
        arts_idx = 1
    elseif f440_local0 == 48 then
        arts_idx = 2
    end
    SetVariable("SwordArtsOneShotShieldCancelCategory", arts_idx)
...
function SwordArtsOneShotShieldBoth_onUpdate()
    ...
    if env(ActionDuration, ACTION_ARM_L2) <= 0 then
        local idle_cat = env(GetStayAnimCategory)
        local wep_cat = env(GetEquipWeaponCategory, c_SwordArtsHand)
        local f442_local0 = GetSwordArtsDiffCategory(c_SwordArtsID, idle_cat, wep_cat)
        local arts_idx = 0
        if f442_local0 == 47 then
            arts_idx = 1
        elseif f442_local0 == 48 then
            arts_idx = 2
        end
        SetVariable("SwordArtsOneShotShieldCancelCategory", arts_idx)
        if TRUE == env(GetSpEffectID, 100285) then
...
function SetSwordArtsWepCategory_DrawStanceRightAttackLight()
    ...
    local wep_cat = env(GetEquipWeaponCategory, c_SwordArtsHand)
    local arts_idx = 0
    if GetSwordArtsDiffCategory(c_SwordArtsID, idle_cat, wep_cat) == 45 then
        arts_idx = 1
    end
...
function VaryArt()  --多才多艺
	local VaryArts = 0
	if env(GetSpEffectID, 103200) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103201) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103202) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103203) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103204) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103205) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103206) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103207) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103208) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103209) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103210) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103211) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103212) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103213) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103214) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103215) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103216) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103217) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103218) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103219) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103220) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103221) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103222) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103223) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103224) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103225) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103226) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103227) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103228) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103229) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103230) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103231) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103232) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103233) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103234) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103235) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103236) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103237) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103238) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103239) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103240) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103241) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103242) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103243) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103244) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103245) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103246) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103247) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103248) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103249) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103250) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103251) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103252) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103253) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103254) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103255) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103256) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103257) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103258) == TRUE then
		VaryArts = VaryArts + 1
	end
	if env(GetSpEffectID, 103259) == TRUE then
		VaryArts = VaryArts + 1
	end
	if VaryArts <= 2 then  --连击种类数
		act(AddSpEffect, 103260)  --连击buff层数1
	elseif VaryArts <= 3 then
		act(AddSpEffect, 103261)  --2
	elseif VaryArts <= 4 then
		act(AddSpEffect, 103262)  --3
	elseif VaryArts <= 5 then
		act(AddSpEffect, 103263)  --4
	elseif VaryArts <= 6 then
		act(AddSpEffect, 103264)  --5
	elseif VaryArts <= 7 then
		act(AddSpEffect, 103265)  --6
	elseif VaryArts <= 8 then
		act(AddSpEffect, 103266)  --7
	elseif VaryArts <= 9 then
		act(AddSpEffect, 103268)  --8
	elseif 10 <= VaryArts then
		act(AddSpEffect, 103269)  --9
	end
	return
end
...
function Act_Jump() 
	...
	-- Vigil
    act(SetMovementScaleMult, g_JumpMovementMultiplier)
    --
...
function JumpCommonFunction(jump_type) 
    ...
    -- Other
        ...
        if GetEquipType(arrowHand, WEAPON_CATEGORY_CROSSBOW) == TRUE then
            if env(GetBoltLoadingState, arrowHand) ~= FALSE then
                if env(IsOutOfAmmo, arrowHand) == TRUE then
				elseif TRUE == env(GetSpEffectID, 10015280) then
					return FALSE
                elseif TRUE == env(GetSpEffectID, 140) and GetVariable("JumpAttackForm") == 0 then
                    if TRUE == env(ActionRequest, ACTION_ARM_R1) or is_arrow == TRUE and TRUE == env(ActionRequest, ACTION_ARM_R2) then
                        ...
                        -- ERR: Powerstance R1 jump attack
                        if IsEnableDualWielding() ~= -1 and env(GetSpEffectID, 165000) == TRUE then
                            SetVariable("JumpAttackFormRequest", 2)
                            SetVariable("JumpAttackForm", 3)
                        end
                    ...
                    elseif TRUE == env(ActionRequest, ACTION_ARM_R2) then
                        ...
                        return TRUE
                        -- ERR: Turn off L1 powerstance jump attack
                    elseif TRUE == env(ActionRequest, ACTION_ARM_L1) and HAND_RIGHT == IsEnableDualWielding() and env(GetSpEffectID, 165000) == FALSE then
                        ExecEventSync("Event_JumpNormalAttack_Add")
                        SetVariable("JumpAttackFormRequest", 2)
                        SetVariable("JumpAttackForm", 3)
                        SetInterruptType(INTERRUPT_FINDATTACK)
                        ...
                        return TRUE
                    elseif TRUE == env(GetSpEffectID, 98) and TRUE == env(IsTruelyLanding) and env(GetSpEffectID, 140) == FALSE and env(GetSpEffectID, 10015280) == FALSE then
                        if jump_type == 0 then
                            SetVariable("JumpAttack_Land", 1)
                ...
                elseif env(GetSpEffectID, 140) == FALSE and GetVariable("JumpAttack_Land") == 0 and GetVariable("JumpAttackForm") == 0 and env(GetSpEffectID, 10015280) == FALSE then
                    SetVariable("JumpAttack_Land", 0)
                    if TRUE == env(ActionRequest, ACTION_ARM_R1) or is_arrow == TRUE and TRUE == env(ActionRequest, ACTION_ARM_R2) then
                        SetVariable("JumpAttackFormRequest", 0)
	...
	if env(GetSpEffectID, 10015280) == TRUE then 
		return FALSE 
    elseif env(GetSpEffectID, 98) == TRUE and env(IsTruelyLanding) == TRUE and env(GetSpEffectID, 140) == FALSE and env(Unknown701) == FALSE then
        local landIndex = GetLandIndex(height, FALSE)
        SetVariable("LandIndex", landIndex)
...
function Jump_Loop_onUpdate() 
    ...
    elseif TRUE == env(ActionRequest, ACTION_ARM_R2) and 0 == GetVariable("JumpAttackForm") then
        ...
        -- ERR: Disable L1 powerstance jump attack
    elseif TRUE == env(ActionRequest, ACTION_ARM_L1) and IsEnableDualWielding() == HAND_RIGHT and env(GetSpEffectID, 165000) == FALSE then
        SetVariable("JumpAttackForm", 3)
        SetVariable("JumpAttackFormRequest", 2)
        hkbFireEvent("W_JumpAttack_Start_Falling")
        return
